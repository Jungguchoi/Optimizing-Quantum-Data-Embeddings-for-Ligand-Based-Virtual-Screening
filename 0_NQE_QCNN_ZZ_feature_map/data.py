...
def data_load_and_process(dataset, feature_reduction='resize256', classes=[0,1]):
    ...
    if dataset == 'protein':
        data_dir = "/Users/jungguchoi/Library/Mobile Documents/com~apple~CloudDocs/1_Post_doc(Cleveland_clinic:2024.10~2025.09)/1_Research_project/3_quantum_embedding_comparison_sequence(2024.09 ~ XXXX.XX)/0_data/2_preprocessed_after_MD/"
        dataset_signal = pd.read_csv(data_dir+"/preprocessed_binary_class.csv")

        dataset_value = dataset_signal.iloc[:,2:]
        dataset_label = dataset_signal.iloc[:,2]
