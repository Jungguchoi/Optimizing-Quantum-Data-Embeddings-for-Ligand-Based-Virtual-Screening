...
def QuantumEmbedding2_with_inverse(input1, input2):
  for i in range(N_layers):
    for j in range(8):
      qml.Hadamard(wires=j)
      exp_Z(input1[j], wires=j)
    for k in range(7):
      exp_ZZ1(input1[8+k], wires=[k, k+1])

  for l in range(N_layers):
    for m in reversed(range(7)):
      exp_ZZ1(input2[m+8], wires=[m,m+1], inverse=True)
    qml.Barrier()
    for n in range(8):
      exp_Z(input2[n], wires=n, inverse=True)
      qml.Hadamard(wires=n)
      
  return qml.probs(wires=range(8))

def repeat_XYZ_embedding_with_inverse(input1, input2):
  for i in range(2):
      for j in range(8):
...
# Quantum Embedding 3 for model 3 (This functions are added by JG)
def QuantumEmbedding3(input):
  for i in range(N_layers):
    for j in range(8):
      qml.Hadamard(wires=j)
      exp_Z(input[j], wires=j)
...
def QuantumEmbedding3_inverse(input):
  for i in range(N_layers):
    #exp_ZZ1(input[15], wires=[7,0], inverse=True)
    for k in reversed(range(7)):
      exp_ZZ1(input[k+8], wires=[k,k+1], inverse=True)
