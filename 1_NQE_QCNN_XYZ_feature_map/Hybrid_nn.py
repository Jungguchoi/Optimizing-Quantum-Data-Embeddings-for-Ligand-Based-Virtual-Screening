...
Model1_Fidelity uses fidelity as a loss function.
Model1_HSinner uses Hilbert-Schmidt inner product as a loss function.
"""
@qml.qnode(dev, interface="torch")
def circuit1(inputs):
    #print("shape of input:", inputs.shape,"/ part of input:", inputs[0:8].shape,"/ part of input:", inputs[8:16].shape)
    embedding.QuantumEmbedding1(inputs[0:4])
    embedding.QuantumEmbedding1_inverse(inputs[4:8])
...
class Model1_Fidelity(torch.nn.Module):
    def __init__(self):
        ...
        self.linear_relu_stack1 = nn.Sequential(
            #torch.nn.Conv1d(in_channels=1, out_channels=1, kernel_size=246, stride=246)
            #torch.nn.ReLU(),
            #torch.nn.Conv1d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1),
            #torch.nn.ReLU(),
            #torch.nn.Conv1d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1),
            #torch.nn.ReLU(),
            torch.nn.Linear(1972, 986),
            torch.nn.ReLU(),
            torch.nn.Linear(986, 4)
        )
    ...
    def forward(self, x1, x2):
        ...
        #print("Input shape of qlayer1:", x.shape)
        x = self.qlayer1(x)
        #print("Output shape of qlayer1:", x.shape)
        return x[:,0]
...
def circuit2(inputs):
    ...
    return embedding.repeat_XYZ_embedding_with_inverse(input1=inputs[:15], input2=inputs[15:])
...
class Model3_HSinner(torch.nn.Module):
    def __init__(self):
        ...
        self.layer2 = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            #torch.nn.MaxPool2d(kernel_size=2, stride=2)
            #torch.nn.Linear(986, 493),
            #torch.nn.ReLU()
        )

        # Fully connected Layers 7 * 7 -> 16
        self.fc1 = torch.nn.Linear(247, 123, bias=True)
        self.fc2 = torch.nn.Linear(123, 16, bias=True)
...
class Model_Amplitude(torch.nn.Module):
    ...
    def forward(self, x1, x0):
        ...
        return torch.prod(torch.cos((x1 - x0)/2), axis=1)
