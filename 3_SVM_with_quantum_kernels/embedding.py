...
# Quantum Embedding 1 for model 1 (Conventional ZZ feature embedding)
def Twelve_QuantumEmbedding1(input):
  for i in range(N_layers):
    for j in range(12):
      qml.Hadamard(wires=j)
      exp_Z(input[j], wires=j)
    for k in range(11):
      exp_ZZ2(input[k], input[k+1], wires=[k,k+1])
    exp_ZZ2(input[11], input[0], wires=[11, 0])

def Twelve_QuantumEmbedding1_inverse(input):
  for i in range(N_layers):
    exp_ZZ2(input[11], input[0], wires=[11, 0], inverse=True)
...
# Quantum Embedding 2 for model 2
def Twelve_QuantumEmbedding2(input):
  for i in range(N_layers):
    for j in range(12):
      qml.Hadamard(wires=j)
      exp_Z(input[j], wires=j)
    for k in range(11):
      exp_ZZ1(input[12+k], wires=[k, k+1])
    exp_ZZ1(input[23], wires=[11,0])

def Twelve_QuantumEmbedding2_inverse(input):
  for i in range(N_layers):
    exp_ZZ1(input[23], wires=[11,0], inverse=True)
    for k in reversed(range(11)):
      exp_ZZ1(input[k+12], wires=[k,k+1], inverse=True)
    qml.Barrier()
    for j in range(12):
      exp_Z(input[j], wires=j, inverse=True)
      qml.Hadamard(wires=j)
...
def XY_ansatz_4_qubits(input):
    for i in range(2):
        for j in range(4):
            qml.RX(input[j], wires=j)
        for k in range(3):
            qml.IsingXX(input[4 + k], wires=[k, k+1])
        qml.Barrier()
        for l in range(4):
            qml.RY(input[7+l], wires=l)
        for m in range(3):
            qml.IsingYY(input[7+4 + m], wires=[m, m+1])

def YZ_ansatz_4_qubits(input):
    for i in range(2):
        for j in range(4):
            qml.RY(input[j], wires=j)
        for k in range(3):
            qml.IsingYY(input[4 + k], wires=[k, k+1])
        qml.Barrier()
        for l in range(4):
            qml.RZ(input[7+l], wires=l)
        for m in range(3):
            qml.IsingZZ(input[7+4 + m], wires=[m, m+1])

def repeat_XYZ_with_inverse_4_qubits(input1, input2):
  for i in range(2):
      for j in range(4):
          qml.RX(input1[j], wires=j)
      for k in range(3):
          qml.IsingXX(input1[4 + k], wires=[k, k+1])
      
      qml.Barrier()

      for l in range(4):
          qml.RY(input1[l], wires=l)
      for m in range(3):
          qml.IsingYY(input1[4 + m], wires=[m, m+1])
      
      qml.Barrier()
      
      for n in range(4):
          qml.RZ(input1[n], wires=n)
      for o in range(3):
          qml.IsingZZ(input1[4 + o], wires=[o, o+1])

  for i in range(2):
      for j in range(4):
          qml.RX(-input2[j], wires=j)
      for k in range(3):
          qml.IsingXX(-input2[4 + k], wires=[k, k+1])
      
      qml.Barrier()
      
      for l in range(4):
          qml.RY(-input2[l], wires=l)
      for m in range(3):
          qml.IsingYY(-input2[4 + m], wires=[m, m+1])
      
      qml.Barrier()
      
      for n in range(4):
          qml.RZ(-input2[n], wires=n)
      for o in range(3):
          qml.IsingZZ(-input2[4 + o], wires=[o, o+1])

def repeat_XYZ_4_qubits(input):
    for i in range(2):
        for j in range(4):
            qml.RX(input[j], wires=j)
        for k in range(3):
        ...
        qml.Barrier()
        for l in range(4):
            qml.RY(input[l], wires=l)
        for m in range(3):
            qml.IsingYY(input[4 + m], wires=[m, m+1])
      
        qml.Barrier()
        for n in range(4):
            qml.RZ(input[n], wires=n)
        for o in range(3):
            qml.IsingZZ(input[4 + o], wires=[o, o+1])
    


def repeat_XYZ_with_inverse_8_qubits(input1, input2):
  for i in range(2):
      for j in range(8):
          qml.RX(input1[j], wires=j)
      for k in range(7):
          qml.IsingXX(input1[8 + k], wires=[k, k+1])
      ...
      for l in range(8):
          qml.RY(input1[l], wires=l)
      for m in range(7):
          qml.IsingYY(input1[8 + m], wires=[m, m+1])
      ...
      for n in range(8):
          qml.RZ(input1[n], wires=n)
      for o in range(7):
          qml.IsingZZ(input1[8 + o], wires=[o, o+1])

  for i in range(2):
      for j in range(8):
          qml.RX(-input2[j], wires=j)
      for k in range(7):
          qml.IsingXX(-input2[8 + k], wires=[k, k+1])
      ...
      for l in range(8):
          qml.RY(-input2[l], wires=l)
      for m in range(7):
          qml.IsingYY(-input2[8 + m], wires=[m, m+1])
      ...
      for n in range(8):
          qml.RZ(-input2[n], wires=n)
      for o in range(7):
          qml.IsingZZ(-input2[8 + o], wires=[o, o+1])

def repeat_XYZ_8_qubits(input):
  for i in range(2):
      for j in range(8):
          qml.RX(input[j], wires=j)
      for k in range(7):
          qml.IsingXX(input[8 + k], wires=[k, k+1])
      ...
      for l in range(8):
          qml.RY(input[l], wires=l)
      for m in range(7):
          qml.IsingYY(input[8 + m], wires=[m, m+1])
      ...
      for n in range(8):
          qml.RZ(input[n], wires=n)
      for o in range(7):
          qml.IsingZZ(input[8 + o], wires=[o, o+1])
...
def QCNN_eight(params, ansatz):
    ...
    param1 = params[0:num_params]
    param2 = params[num_params:2 * num_params]
    param3 = params[2*num_params:3*num_params]
    U(param1, wires=[0, 1])
    U(param1, wires=[2, 3])
    U(param1, wires=[4, 5])
    U(param1, wires=[6, 7])
...
def QCNN_twelve(params, ansatz):
    ...
    param2 = params[num_params:2 * num_params]
    param3 = params[2*num_params:3*num_params]
    param4 = params[3*num_params:4*num_params]
    U(param1, wires=[0, 1])
    U(param1, wires=[2, 3])
    U(param1, wires=[4, 5])
    U(param1, wires=[6, 7])
    U(param1, wires=[8, 9])
    U(param1, wires=[10, 11])
    U(param1, wires=[1, 2])
    U(param1, wires=[3, 4])
    U(param1, wires=[5, 6])
    U(param1, wires=[7, 8])
...
def Noisy_QCNN_eight(params, ansatz):
    if ansatz == 'SU4':
      U = U_SU4
      num_params = 15
    elif ansatz == 'TTN':
